...
local function DisplayNotification(msg)
	if TELESCOPE.UseNativeNotifiactions then
...
local function SetupInstructions()
	...
	AddTextComponentSubstringPlayerName("QUITTER LE TÉLESCOPE")
...
local function CreateTelescopeCamera(entity, data)
    ...
    BeginScaleformMovieMethod(scaleform, "SET_DISPLAY_TELESCOPE")
...
local function IsPedPlayingAnyTelescopeAnim(ped)
    for animType, animations in pairs(TELESCOPE.Animations) do 
...
local function HandleZoom()
	if GetDisabledControlNormal(0, 32) ~= 0.0 or GetDisabledControlNormal(0, 335) ~= 0.0 then -- Zoom in
        fov = math.max(fov - TELESCOPE.Zoom.Speed, TELESCOPE.Zoom.Min)
    end
    if GetDisabledControlNormal(0, 33) ~= 0.0 or GetDisabledControlNormal(0, 336) ~= 0.0 then -- Zoom out
        fov = math.min(fov + TELESCOPE.Zoom.Speed, TELESCOPE.Zoom.Max)
    end
...
local function HandleMovement(maxVertical)
	...
	if rightAxisX ~= 0.0 or rightAxisY ~= 0.0 then
        local zoomValue = (1.0/(TELESCOPE.Zoom.Max-TELESCOPE.Zoom.Min))*(fov-TELESCOPE.Zoom.Min)
        local rotation = GetCamRot(camera, 2)
        local heading = RotationToHeading(rotation.z)
        
        local movementSpeed = (IsUsingKeyboard(1) and TELESCOPE.MovementSpeed.Keyboard) or TELESCOPE.MovementSpeed.Controller
...
local function GetClosestTelescope()
    local closest = 0
    local closestDist = TELESCOPE.MaxDetectionDist
    local coords = GetEntityCoords(PlayerPedId())

    for model, data in pairs(TELESCOPE.Models) do
        local entity = GetClosestObjectOfType(coords.x, coords.y, coords.z, TELESCOPE.MaxDetectionDist, model, false, false, false)
        if entity ~= 0 then
...
local function UseTelescope(entity)
    ...
    if not IsTelescopeAvailable(offsetCoords) then
        DisplayNotification(TELESCOPE.TelescopeInUse)
        return
    end
    ...
    elseif dist > 2.5 then
        DisplayNotification(TELESCOPE.ToFarAway)
        ClearPedTasks(playerPed)
        return
    end
...
-- Help Text Thread --
if TELESCOPE.UseDistanceThread then
    Citizen.CreateThread(function()
        while true do
            local sleep = 500
            local coords = GetEntityCoords(PlayerPedId())
            ...
            if not inTelescope then
                ...
                if closest ~= 0 then
                    if distance < TELESCOPE.MaxInteractionDist then
                        sleep = 0
                        DisplayHelpText(TELESCOPE.HelpText)
                        if IsControlJustPressed(0, 46) then
                            local telescope = GetClosestTelescope()
                            if telescope ~= 0 then
                                UseTelescope(telescope)
                            else
                                DisplayNotification(TELESCOPE.NoFoundMessage)
                            end
                        end
                    else
                        sleep = distance*20
