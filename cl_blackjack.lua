            ...
            if closestChair ~= nil and closestChairDist < 2 then
                ...
                if not timeoutHowToBlackjack then
                    if blackjackTableData[closestChair] == false then
                        ...
                        ESX.ShowHelpNotification("Appuyez sur E pour jouer au blackjack")
                    else
...
Citizen.CreateThread(function()
    while true do 
        ...
        if drawCurrentHand then
            ...
            AddTextComponentString("Votre main est [~y~"..tostring(currentHand).."~s~], la main du croupier est [~y~"..tostring(dealersHand).."~s~]")
        ...
        if waitingForBetState then
            ...
            if IsDisabledControlJustPressed(0, 22) then --Custom Bet [space]
                ...
                if tonumber(tmpInput) then
                    if tmpInput <= maxBet then
                        tmpInput = tonumber(tmpInput) 
                        if tmpInput > 0 then
                            currentBetAmount = tmpInput
                        end
                    else
                        ESX.ShowNotification("~r~Limite de mise atteinte (20,000 jetons)")
                    end
                end
            end
            ...
            if IsControlJustPressed(0, 201) then --Submit bet [Enter]
                ...
                if tonumber(currentBetAmount) >= 0 and currentBetAmount <= maxBet then
            ...
            if IsControlPressed(0, 11) and GetGameTimer()-startTime > 250 then --Decrease bet [pagedown]
                if currentBetAmount >= 100 then 
                    ...
                    notify('~r~Mise minimum atteinte')
                end
            end            
        end
        Wait(interval)
...
Citizen.CreateThread(function()
    while true do
        local pNear = false
        if closestChair ~= -1 and closestChairDist < 3 then 
            pNear = true
    ...
    while true do
        ...
        if blackjackInstructional ~= nil then 
            ...
            if waitingForBetState then 
                ...
                SlotDrawAdvancedText(2, 'Mise', currentBetAmount)
            end
        end

        if drawTimerBar then
            pNear = true
            SlotDrawAdvancedText(3, 'Temps', (timeLeft < 10 and "00:0" or "00:") .. tostring(math.floor(timeLeft)))
        end
...
AddEventHandler("Blackjack:beginBetsBlackjack",function(gameID,tableId)
    ...
    ClearHelp(true)
    drawNativeNotification("Placez vos mises")
    ...
    Citizen.CreateThread(function()
        ...
        if not bettedThisRound then
            ...
            drawNativeNotification("Aucune mise placée, tour sauté")
        end
    end)
end)
...
function goToBlackjackSeat(blackjackSeatID)
    ...
    drawNativeNotification("Attendez que le prochain jeu commence...")
    blackjackAnimsToLoad = {
      "anim_casino_b@amb@casino@games@blackjack@dealer",
      "anim_casino_b@amb@casino@games@shared@dealer@",
      "anim_casino_b@amb@casino@games@blackjack@player",  
    ...
    if shouldForceIdleCardGames and sittingAtBlackjackTable then
        TaskPlayAnim(PlayerPedId(),"anim_casino_b@amb@casino@games@shared@player@", "idle_cardgames", 1.0, 1.0, -1, 1)
    end
end 
...
AddEventHandler("Blackjack:blackjackPush",function(tableId)
    if closeToCasino then
        ...
        annoyedIPushed()
        canExitBlackjack = true
        drawNativeNotification("~b~Vous avez poussé")
        drawCurrentHand = false
        currentHand = 0
        dealersHand = 0
    end
...
AddEventHandler("Blackjack:blackjackWin",function(tableId)
    if closeToCasino then
        ...
        happyIWon()
        canExitBlackjack = true
        drawNativeNotification("~y~Vous avez gagné")
        drawCurrentHand = false
        currentHand = 0
        dealersHand = 0
    end
...
function setupBlackjackMidBetScaleform(scaleform)
    ...
    PushScaleformMovieFunctionParameterInt(1)
    Button(GetControlInstructionalButton(2, 194, true)) -- The button to display
    ButtonMessage("Rester") --BACKSPACE
    PopScaleformMovieFunctionVoid()

    PushScaleformMovieFunction(scaleform, "SET_DATA_SLOT")
    PushScaleformMovieFunctionParameterInt(4)
    Button(GetControlInstructionalButton(2, 22, true))
    ButtonMessage("Tirer") --Space
    PopScaleformMovieFunctionVoid()  
