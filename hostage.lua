...
RegisterCommand("otage",function()
	local inKothPrep = exports["rg_core"]:inKothPrep()
	if not inKothPrep then
		takeHostage()
	end
end)
...
function takeHostage()
	...
	if not canTakeHostage then 
		drawNativeNotification("Vous devez avoir un pistolet !")
	end
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"Rêlacher:[G] / Tuer:[H]")
