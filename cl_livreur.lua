...
local missionVeh = 0         -- véhicule spawn pour le joueur
local lastSelect = 0
...
-- Liste des points de livraison supplémentaires
...
-- Ajoute chaque coordonnée à la liste des livraisons du job livreur
                        ...
                        RageUI.ButtonWithStyle(("• %s"):format(d.label), ("Rémunération: ~g~$%d"):format(d.payout),
                            {RightLabel="Accepter"}, true, function(_,Active,Selected)
                                if Selected then
                                    ...
                                    -- Prévenir le serveur (verrou anti-exploit)
