...
RegisterNetEvent('aquiverPoker:updatePokerPlayerChips')
...
AddEventHandler('aquiverPoker:updatePokerPlayerChips', function(amount)
    if amount ~= nil then
        PlayerOwnedChips = amount
    else
        Config3Poker.DebugMsg('Error in: aquiverPoker:updatePokerPlayerChips. (Your param[amount] is nil)')
    end
end)

AddEventHandler(
    'aquiverPoker:playerPokerDraw',
    function(tableId)
        if SharedPokers[tableId] ~= nil then
...
AddEventHandler(
    ...
    function(tableId)
        if SharedPokers[tableId] ~= nil then
            currentHelpText = 'Rangement de la table..\n~b~La prochaine partie commence bientôt.\n'
...
AddEventHandler(
    ...
    function(tableId)
        if SharedPokers[tableId] ~= nil then
            currentHelpText = 'Le ~r~Croupier~w~ révèle sa main.\n'
...
AddEventHandler(
    ...
    function(tableId)
        if SharedPokers[tableId] ~= nil then
            currentHelpText = 'Révélation des mains des joueurs..\n'
...
AddEventHandler(
    ...
    function(tableId)
        if SharedPokers[tableId] ~= nil then
            currentHelpText = 'Distribution des cartes aux joueurs..\n'
...
AquiverPoker = function(index, data)
    ...
    self.playerDraw = function()
        ...
        if self.isPedFemale() then
            ...
            TaskSynchronizedScene(self.ped, pedReaction, Config3Poker.DealerAnimDictShared, pedr, 2.0, -2.0, 13, 16, 1000.0, 0)
        else
    ...
    self.playerWin = function()
        ...
        if reaction then
            ...
            NetworkAddPedToSynchronisedScene(PlayerPedId(), reactionScene, Config3Poker.PlayerAnimDictShared, reaction, 2.0, -2.0, 13, 16, 2.0, 0)
    ...
    self.createDefaultPakli = function()
        Citizen.CreateThread(
            function()
                ...
                RequestAnimDict(Config3Poker.DealerAnimDictPoker)
                while not HasAnimDictLoaded(Config3Poker.DealerAnimDictPoker) do
                    Citizen.Wait(1)
                end

                local offset = GetAnimInitialOffsetPosition(Config3Poker.DealerAnimDictPoker, 'deck_pick_up_deck', self.data.Position, 0.0, 0.0, self.data.Heading, 0.01, 2)
    ...
    self.createPed = function()
        Citizen.CreateThread(
            function()
                ...
                RequestAnimDict(Config3Poker.DealerAnimDictShared)
                while not HasAnimDictLoaded(Config3Poker.DealerAnimDictShared) do
                    Citizen.Wait(1)
                end

                self.dealerStandingIdle()

                Config3Poker.DebugMsg('Poker ped created.')
            end
        )
    end
    ...
    self.sitDownCallback = function(chairId, chairCoords, chairRotation)
        ...
        RequestAnimDict(Config3Poker.PlayerAnimDictShared)
        while not HasAnimDictLoaded(Config3Poker.PlayerAnimDictShared) do
            Citizen.Wait(1)
        end
        ...
        NetworkAddPedToSynchronisedScene(PlayerPedId(), sitScene, Config3Poker.PlayerAnimDictShared, sitAnim, 2.0, -2.0, 13, 16, 2.0, 0)
        ...
        NetworkAddPedToSynchronisedScene(PlayerPedId(), mainScene, Config3Poker.PlayerAnimDictShared, 'idle_cardgames', 2.0, -2.0, 13, 16, 1000.0, 0)
    ...
    self.FirstAction = function()
        ...
        if self.isPedFemale() then
            TaskSynchronizedScene(self.ped, firstScene, Config3Poker.DealerAnimDictPoker, 'female_deck_pick_up', 2.0, -2.0, 13, 16, 1000.0, 0)
        else
            TaskSynchronizedScene(self.ped, firstScene, Config3Poker.DealerAnimDictPoker, 'deck_pick_up', 2.0, -2.0, 13, 16, 1000.0, 0)
        end
        ...
        if self.ServerCards['dealer'] ~= nil then
            ...
            if not DoesEntityExist(self.cards['dealer'][1]) then
                self.cards['dealer'][1] = self.createCard(Config3Poker.Cards[self.ServerCards['dealer'].Hand[1]])
            end
            if not DoesEntityExist(self.cards['dealer'][2]) then
                self.cards['dealer'][2] = self.createCard(Config3Poker.Cards[self.ServerCards['dealer'].Hand[2]])
            end
            if not DoesEntityExist(self.cards['dealer'][3]) then
                self.cards['dealer'][3] = self.createCard(Config3Poker.Cards[self.ServerCards['dealer'].Hand[3]])
            end
        end

        local secondScene = CreateSynchronizedScene(self.data.Position, 0.0, 0.0, self.data.Heading, 2)
        if self.isPedFemale() then
            TaskSynchronizedScene(self.ped, secondScene, Config3Poker.DealerAnimDictPoker, 'female_deck_shuffle', 2.0, -2.0, 13, 16, 1000.0, 0)
        else
            TaskSynchronizedScene(self.ped, secondScene, Config3Poker.DealerAnimDictPoker, 'deck_shuffle', 2.0, -2.0, 13, 16, 1000.0, 0)
        end
        PlaySynchronizedEntityAnim(self.cards['dealer'][1], secondScene, 'deck_shuffle_card_a', Config3Poker.DealerAnimDictPoker, 1000.0, 0, 0, 1000.0)
        PlaySynchronizedEntityAnim(self.cards['dealer'][2], secondScene, 'deck_shuffle_card_b', Config3Poker.DealerAnimDictPoker, 1000.0, 0, 0, 1000.0)
        PlaySynchronizedEntityAnim(self.cards['dealer'][3], secondScene, 'deck_shuffle_card_c', Config3Poker.DealerAnimDictPoker, 1000.0, 0, 0, 1000.0)
        ...
        if self.isPedFemale() then
            TaskSynchronizedScene(self.ped, thirdScene, Config3Poker.DealerAnimDictPoker, 'female_deck_idle', 2.0, -2.0, 13, 16, 1000.0, 0)
        else
            TaskSynchronizedScene(self.ped, thirdScene, Config3Poker.DealerAnimDictPoker, 'deck_idle', 2.0, -2.0, 13, 16, 1000.0, 0)
        end
    ...
    self.dealToPlayers = function()
        ...
        buttonScaleform = nil
        Config3Poker.DebugMsg('dealing to players')
        ...
        for targetSrc, data in pairs(self.ServerCards) do
            if targetSrc ~= 'dealer' then
                ...
                self.cards[targetSrc][1] = self.createCard(Config3Poker.Cards[data.Hand[1]])
                self.cards[targetSrc][2] = self.createCard(Config3Poker.Cards[data.Hand[2]])
                self.cards[targetSrc][3] = self.createCard(Config3Poker.Cards[data.Hand[3]])
                ...
                if playerAnimId ~= nil then
                    ...
                    if self.isPedFemale() then
                        TaskSynchronizedScene(self.ped, dealScene, Config3Poker.DealerAnimDictPoker, string.format('female_deck_deal_%s', playerAnimId), 2.0, -2.0, 13, 16, 1000.0, 0)
                    else
                        TaskSynchronizedScene(self.ped, dealScene, Config3Poker.DealerAnimDictPoker, string.format('deck_deal_%s', playerAnimId), 2.0, -2.0, 13, 16, 1000.0, 0)
                    end
                    ...
                    PlaySynchronizedEntityAnim(
                        ...
                        Config3Poker.DealerAnimDictPoker,
                        1000.0,
                        0,
                        0,
                        1000.0
        ...
        Config3Poker.DebugMsg('dealing ended')
    end
    ...
    self.watchCards = function()
        ...
        if self.index == activePokerTable and playerBetted ~= nil then
            clientTimer = Config3Poker.PlayerDecideTime
            Citizen.CreateThread(
                function()
                    while clientTimer ~= nil do
                        ...
                        if clientTimer ~= nil then
                            ...
                            if clientTimer < 1 then
                                clientTimer = nil
                                ESX.ShowNotification('Vous n\'avez pas répondu à la demande du croupier à temps, vous avez couché votre main.')
        ...
        RequestAnimDict(Config3Poker.PlayerAnimDictPoker)
        while not HasAnimDictLoaded(Config3Poker.PlayerAnimDictPoker) do
            Citizen.Wait(1)
        end
        ...
        for targetSrc, data in pairs(self.ServerCards) do
            if targetSrc ~= 'dealer' then
                ...
                if GetPlayerServerId(PlayerId()) == targetSrc and self.index == activePokerTable then
                    ...
                    NetworkAddPedToSynchronisedScene(PlayerPedId(), scene, Config3Poker.PlayerAnimDictPoker, 'cards_pickup', 2.0, -2.0, 13, 16, 1000.0, 0)
                    ...
                    Citizen.CreateThread(
                        function()
                            ...
                            local playerHandValue = Config3Poker.getHandAllValues(data.Hand)
                            if playerHandValue ~= nil then
                                Config3Poker.DebugMsg(string.format('Player hand value: %s', playerHandValue))
                                local form = Config3Poker.formatHandValue(playerHandValue)
                                if form ~= nil then
                                    Citizen.CreateThread(
                                        function()
                ...
                PlaySynchronizedEntityAnim(self.cards[targetSrc][1], cardsScene, 'cards_pickup_card_a', Config3Poker.PlayerAnimDictPoker, 1000.0, 0, 0, 1000.0)
                PlaySynchronizedEntityAnim(self.cards[targetSrc][2], cardsScene, 'cards_pickup_card_b', Config3Poker.PlayerAnimDictPoker, 1000.0, 0, 0, 1000.0)
                PlaySynchronizedEntityAnim(self.cards[targetSrc][3], cardsScene, 'cards_pickup_card_c', Config3Poker.PlayerAnimDictPoker, 1000.0, 0, 0, 1000.0)
            end
        end
    end
    ...
    self.foldCards = function(mainSrc)
        ...
        if GetPlayerServerId(PlayerId()) == mainSrc then
            ...
            NetworkAddPedToSynchronisedScene(PlayerPedId(), scene, Config3Poker.PlayerAnimDictPoker, 'cards_fold', 2.0, -2.0, 13, 16, 1000.0, 0)
            NetworkStartSynchronisedScene(scene)
        ...
        if self.cards[mainSrc] ~= nil then
            ...
            PlaySynchronizedEntityAnim(self.cards[mainSrc][1], cardsScene, 'cards_fold_card_a', Config3Poker.PlayerAnimDictPoker, 1000.0, 0, 0, 1000.0)
            PlaySynchronizedEntityAnim(self.cards[mainSrc][2], cardsScene, 'cards_fold_card_b', Config3Poker.PlayerAnimDictPoker, 1000.0, 0, 0, 1000.0)
            PlaySynchronizedEntityAnim(self.cards[mainSrc][3], cardsScene, 'cards_fold_card_c', Config3Poker.PlayerAnimDictPoker, 1000.0, 0, 0, 1000.0)
        end
    end
    ...
    self.playCards = function(mainSrc)
        if GetPlayerServerId(PlayerId()) == mainSrc then
            ...
            Citizen.CreateThread(
                function()
                    local scene = NetworkCreateSynchronisedScene(activeChairData.chairCoords, activeChairData.chairRotation, 2, true, false, 1.0, 0.0, 1.0)
                    NetworkAddPedToSynchronisedScene(PlayerPedId(), scene, Config3Poker.PlayerAnimDictPoker, 'cards_play', 2.0, -2.0, 13, 16, 1000.0, 0)
                    NetworkStartSynchronisedScene(scene)
                    ...
                    NetworkAddPedToSynchronisedScene(PlayerPedId(), nextScene, Config3Poker.PlayerAnimDictPoker, 'cards_bet', 2.0, -2.0, 13, 16, 1000.0, 0)
                    ...
                    if offsetAlign == nil then
                        Config3Poker.DebugMsg('Something error happened during the playCards function.')
                        return
                    end
        ...
        if self.cards[mainSrc] ~= nil and self.ServerCards[mainSrc] ~= nil then
            local chairData = self.ServerCards[mainSrc].chairData
            local cardsScene = CreateSynchronizedScene(chairData.chairCoords, chairData.chairRotation, 2)
            PlaySynchronizedEntityAnim(self.cards[mainSrc][1], cardsScene, 'cards_play_card_a', Config3Poker.PlayerAnimDictPoker, 1000.0, 0, 0, 1000.0)
            PlaySynchronizedEntityAnim(self.cards[mainSrc][2], cardsScene, 'cards_play_card_b', Config3Poker.PlayerAnimDictPoker, 1000.0, 0, 0, 1000.0)
            PlaySynchronizedEntityAnim(self.cards[mainSrc][3], cardsScene, 'cards_play_card_c', Config3Poker.PlayerAnimDictPoker, 1000.0, 0, 0, 1000.0)
        end
    end
    ...
    self.playerRandomIdleAnim = function()
        ...
        if selectedIdleAnim ~= nil then
            ...
            NetworkAddPedToSynchronisedScene(PlayerPedId(), playerIdleScene, Config3Poker.PlayerAnimDictShared, selectedIdleAnim, 2.0, -2.0, 13, 16, 1000.0, 0)
            ...
            NetworkAddPedToSynchronisedScene(PlayerPedId(), playerIdleScene2, Config3Poker.PlayerAnimDictShared, 'idle_cardgames', 2.0, -2.0, 13, 16, 1000.0, 0)
            NetworkStartSynchronisedScene(playerIdleScene2)
        end
    end

    self.dealToSelf = function()
        Config3Poker.DebugMsg('dealing dealer cards.')
        local dealSelfScene = CreateSynchronizedScene(self.data.Position, 0.0, 0.0, self.data.Heading, 2)
        if self.isPedFemale() then
            TaskSynchronizedScene(self.ped, dealSelfScene, Config3Poker.DealerAnimDictPoker, 'female_deck_deal_self', 2.0, -2.0, 13, 16, 1000.0, 0)
        else
            TaskSynchronizedScene(self.ped, dealSelfScene, Config3Poker.DealerAnimDictPoker, 'deck_deal_self', 2.0, -2.0, 13, 16, 1000.0, 0)
        end
        PlaySynchronizedEntityAnim(self.cards['dealer'][1], dealSelfScene, 'deck_deal_self_card_a', Config3Poker.DealerAnimDictPoker, 1000.0, 0, 0, 1000.0)
        PlaySynchronizedEntityAnim(self.cards['dealer'][2], dealSelfScene, 'deck_deal_self_card_b', Config3Poker.DealerAnimDictPoker, 1000.0, 0, 0, 1000.0)
        PlaySynchronizedEntityAnim(self.cards['dealer'][3], dealSelfScene, 'deck_deal_self_card_c', Config3Poker.DealerAnimDictPoker, 1000.0, 0, 0, 1000.0)
    ...
    self.dealerStandingIdle = function()
        ...
        if self.isPedFemale() then
            TaskSynchronizedScene(self.ped, scene, Config3Poker.DealerAnimDictShared, 'female_idle', 1000.0, -2.0, -1.0, 33, 1000.0, 0)
        else
            TaskSynchronizedScene(self.ped, scene, Config3Poker.DealerAnimDictShared, 'idle', 1000.0, -2.0, -1.0, 33, 1000.0, 0)
        end
    end
    ...
    self.putDownDeck = function()
        local scene = CreateSynchronizedScene(self.data.Position, 0.0, 0.0, self.data.Heading, 2)
        if self.isPedFemale() then
            TaskSynchronizedScene(self.ped, scene, Config3Poker.DealerAnimDictPoker, 'female_deck_put_down', 2.0, -2.0, 13, 16, 1000.0, 0)
        else
            TaskSynchronizedScene(self.ped, scene, Config3Poker.DealerAnimDictPoker, 'deck_put_down', 2.0, -2.0, 13, 16, 1000.0, 0)
        end
        ...
        if IsEntityAttachedToAnyPed(self.pakli) then
            ...
            FreezeEntityPosition(self.pakli, true)
            Config3Poker.DebugMsg('pakli detached')
        end
    ...
    self.EnableRender = function(state)
        if state then
                    ...
                    while activePokerTable do
                        ...
                        if playerBetted then
                            ...
                            if self.TimeLeft > 0 then
                                reactiveText = reactiveText .. 'En attente de ~b~joueurs~w~...\n'
                            end
                            ...
                            if self.TimeLeft == nil or self.TimeLeft > 0 then
                                    ...
                                    if tonumber(tmpInput) then
                                        ...
                                        if tmpInput > 0 then
                                            if tmpInput > self.data.MaximumBet then
                                                ...
                                                local betDifference = tmpInput - tmpInput
                                                currentBetInput = tmpInput
                                                PlayerOwnedChips = PlayerOwnedChips - betDifference
                                ...
                                if IsDisabledControlJustPressed(0, 176) then
                                    if currentBetInput > 0 then
                                        ...
                                        ESX.ShowNotification('Vous n\'avez pas défini de mise.')
                                    end
                                end

                                if IsDisabledControlJustPressed(0, 172) then -- up
                                    local increase = Config3Poker.IncreaseAmounts(currentBetInput)
                                    local oldBet = currentBetInput
                                    ...
                                    PlayerOwnedChips = PlayerOwnedChips - (currentBetInput - oldBet)
                        ...
                        if self.Active then
                            if self.TimeLeft >= 10 then
                                ...
                                DrawAdvancedNativeText(1.013, 0.806, 0.005, 0.0028, 0.29, 'TEMPS:', 255, 255, 255, 255, 0, 0)
                        ...
                        DrawAdvancedNativeText(0.965, 0.892, 0.005, 0.0028, 0.29, 'JETONS:', 255, 255, 255, 255, 0, 0)
                        DrawAdvancedNativeText(1.041, 0.885, 0.005, 0.0028, 0.464, string.format('%s', ESX.Math.GroupDigits(PlayerOwnedChips)), 255, 255, 255, 255, 0, 0)

                        DrawRect(0.91, 0.928, 0.145, 0.032, 0, 0, 0, 200)
                        DrawAdvancedNativeText(0.965, 0.935, 0.005, 0.0028, 0.29, 'MISE:', 255, 255, 255, 255, 0, 0)
                        DrawAdvancedNativeText(1.041, 0.928, 0.005, 0.0028, 0.464, string.format('%s', ESX.Math.GroupDigits(currentBetInput)), 255, 255, 255, 255, 0, 0)
                    end
                end
            )
        else
            ...
            NetworkAddPedToSynchronisedScene(PlayerPedId(), sitExitScene, Config3Poker.PlayerAnimDictShared, 'sit_exit_left', 2.0, -2.0, 13, 16, 2.0, 0)
    ...
    self.revealSelfCards = function()
        Citizen.CreateThread(
            function()
                if self.index == activePokerTable then
                    ...
                    local dealerHandValue = Config3Poker.getHandAllValues(self.ServerCards['dealer'].Hand)
                    if dealerHandValue ~= nil then
                        Config3Poker.DebugMsg(string.format('Dealer hand value: %s', dealerHandValue))
                        local form = Config3Poker.formatHandValue(dealerHandValue)
                        if form ~= nil then
                            Citizen.CreateThread(
                                function()
        ...
        if self.ServerCards['dealer'] ~= nil then
            ...
            if self.isPedFemale() then
                TaskSynchronizedScene(self.ped, revealScene, Config3Poker.DealerAnimDictPoker, 'female_reveal_self', 2.0, -2.0, 13, 16, 1000.0, 0)
            else
                TaskSynchronizedScene(self.ped, revealScene, Config3Poker.DealerAnimDictPoker, 'reveal_self', 2.0, -2.0, 13, 16, 1000.0, 0)
            end
            PlaySynchronizedEntityAnim(self.cards['dealer'][1], revealScene, 'reveal_self_card_a', Config3Poker.DealerAnimDictPoker, 1000.0, 0, 0, 1000.0)
            PlaySynchronizedEntityAnim(self.cards['dealer'][2], revealScene, 'reveal_self_card_b', Config3Poker.DealerAnimDictPoker, 1000.0, 0, 0, 1000.0)
            PlaySynchronizedEntityAnim(self.cards['dealer'][3], revealScene, 'reveal_self_card_c', Config3Poker.DealerAnimDictPoker, 1000.0, 0, 0, 1000.0)
        end
    end
            ...
            if targetSrc ~= 'dealer' then
                ...
                if mainAnimFormat ~= nil then
                    if activePokerTable == self.index then -- only show camera if he/she is sitting at the table.
                        if Config3Poker.ShowCardsAfterReveal then
                            local offset =
                                GetAnimInitialOffsetPosition(Config3Poker.PlayerAnimDictPoker, 'cards_play_card_b', data.chairData.chairCoords, data.chairData.chairRotation, 0.0, 2)
                    ...
                    local revealScene = CreateSynchronizedScene(self.data.Position, 0.0, 0.0, self.data.Heading, 2)
                    TaskSynchronizedScene(self.ped, revealScene, Config3Poker.DealerAnimDictPoker, mainAnimFormat, 2.0, -2.0, 13, 16, 1000.0, 0)
                    PlaySynchronizedEntityAnim(self.cards[targetSrc][1], revealScene, entityAnimFormatA, Config3Poker.DealerAnimDictPoker, 1000.0, 0, 0, 1000.0)
                    PlaySynchronizedEntityAnim(self.cards[targetSrc][2], revealScene, entityAnimFormatB, Config3Poker.DealerAnimDictPoker, 1000.0, 0, 0, 1000.0)
                    PlaySynchronizedEntityAnim(self.cards[targetSrc][3], revealScene, entityAnimFormatC, Config3Poker.DealerAnimDictPoker, 1000.0, 0, 0, 1000.0)
                    ...
                    if self.isPedFemale() then
                        TaskSynchronizedScene(self.ped, ggScene, Config3Poker.DealerAnimDictShared, string.format('female_acknowledge_%s', playerAnimId), 2.0, -2.0, 13, 16, 1000.0, 0)
                    else
                        TaskSynchronizedScene(self.ped, ggScene, Config3Poker.DealerAnimDictShared, string.format('acknowledge_%s', playerAnimId), 2.0, -2.0, 13, 16, 1000.0, 0)
                    end
                end
            end
        end
    ...
    self.clearTable = function()
        ...
        for targetSrc, data in pairs(self.ServerCards) do
            if targetSrc ~= 'dealer' then
                ...
                if self.isPedFemale() then
                    TaskSynchronizedScene(self.ped, collectScene, Config3Poker.DealerAnimDictPoker, string.format('female_cards_collect_%s', playerAnimId), 2.0, -2.0, 13, 16, 1000.0, 0)
                else
                    TaskSynchronizedScene(self.ped, collectScene, Config3Poker.DealerAnimDictPoker, string.format('cards_collect_%s', playerAnimId), 2.0, -2.0, 13, 16, 1000.0, 0)
                end
                PlaySynchronizedEntityAnim(
                    self.cards[targetSrc][1],
                    collectScene,
        ...
        if self.ServerCards['dealer'] then
            local collectScene = CreateSynchronizedScene(self.data.Position, 0.0, 0.0, self.data.Heading, 2)
            if self.isPedFemale() then
                TaskSynchronizedScene(self.ped, collectScene, Config3Poker.DealerAnimDictPoker, 'female_cards_collect_self', 2.0, -2.0, 13, 16, 1000.0, 0)
            else
                TaskSynchronizedScene(self.ped, collectScene, Config3Poker.DealerAnimDictPoker, 'cards_collect_self', 2.0, -2.0, 13, 16, 1000.0, 0)
            end
            PlaySynchronizedEntityAnim(self.cards['dealer'][1], collectScene, 'cards_collect_self_card_a', Config3Poker.DealerAnimDictPoker, 1000.0, 0, 0, 1000.0)
            PlaySynchronizedEntityAnim(self.cards['dealer'][2], collectScene, 'cards_collect_self_card_b', Config3Poker.DealerAnimDictPoker, 1000.0, 0, 0, 1000.0)
            PlaySynchronizedEntityAnim(self.cards['dealer'][3], collectScene, 'cards_collect_self_card_c', Config3Poker.DealerAnimDictPoker, 1000.0, 0, 0, 1000.0)
            SetBit(0)
            while GetSynchronizedScenePhase(collectScene) < 0.99 do
                Citizen.Wait(1)
            end
    ...
    self.playerPairPlusAnim = function(amount)
        ...
        if offsetAlign == nil then
            Config3Poker.DebugMsg('Something error happened during the playerBetAnim function.')
            return
        end
        ...
        local scene = NetworkCreateSynchronisedScene(activeChairData.chairCoords, activeChairData.chairRotation, 2, true, false, 1.0, 0.0, 1.0)
        NetworkAddPedToSynchronisedScene(PlayerPedId(), scene, Config3Poker.PlayerAnimDictPoker, animName, 2.0, -2.0, 13, 16, 1000.0, 0)
        NetworkStartSynchronisedScene(scene)
...
Citizen.CreateThread(
    function()
        while true do
            ...
            closeToPokers = false
            for k, v in pairs(Config3Poker.Pokers) do
...
Citizen.CreateThread(
    function()
        ...
        Config3Poker.DebugMsg('Poker loading started..')
        for index, data in pairs(Config3Poker.Pokers) do
            ...
            if not InformationPlaying and activePokerTable == nil and activeChairData == nil then
                if closeToPokers then
                    ...
                    for k, v in pairs(SharedPokers) do
                        ...
                        if dist < 3.0 then
                            for i = 1, #Config3Poker.Tables, 1 do
                                local tableObj = GetClosestObjectOfType(playerpos, 3.0, GetHashKey(Config3Poker.Tables[i]), false)

                                if DoesEntityExist(tableObj) then
                                    letSleep = false

                                    for chairBone, chairId in pairs(Config3Poker.PokerChairs) do
                                        local chaircoords = GetWorldPositionOfEntityBone(tableObj, GetEntityBoneIndexByName(tableObj, chairBone))
                                        if chaircoords then
                                            if #(playerpos - chaircoords) < 1.5 and (not closeTable.ChairCoords or #(playerpos - chaircoords) < #(playerpos - closeTable.ChairCoords))then
                                                local chairrotation = GetWorldRotationOfEntityBone(tableObj, GetEntityBoneIndexByName(tableObj, chairBone))
                                                close = true
                                                closeTable = {Poker = v, ChairId = chairId, ChairCoords = chaircoords, ChairRotation = chairrotation}
                                            end
                                        end
                                    end
                                    if closeTable.ChairCoords then
                    ...
                    if close then
                        if not instruction_showed then
                            instruction_showed = true
                        end
                    else
                        -- if called then
                        --     called = false
...
Citizen.CreateThread(function()
    while true do
        ...
        if closeTable.ChairCoords then
            if #(GetEntityCoords(PlayerPedId()) - closeTable.ChairCoords) <= 2.0 then
                ...
                if activePokerTable == nil then
                    ESX.ShowHelpNotification("Appuyez sur E pour jouer au poker")
                end

                if IsControlJustPressed(0, 38) then
                    closeTable.Poker.sitDown(closeTable.ChairId, closeTable.ChairCoords, closeTable.ChairRotation)
                end
            else
                -- RageUI.CloseAll()
                CASINO["nearThing"] = false
                instruction_showed = false
            end
        end
...
function Show3PokerMenu()
    instruction_showed = true
	-- PlaySoundFrontend(-1, "DLC_VW_RULES", "dlc_vw_table_games_frontend_sounds", true)

	local coords = GetEntityCoords(PlayerPedId())

	RMenu.Add('casino', 'main', RageUI.CreateMenu("", "THREE CARD POKER", 1, 100, closeTable.Poker.data.HighStake and 'CasinoUI_Cards_Three_High' or 'CasinoUI_Cards_Three', closeTable.Poker.data.HighStake and 'CasinoUI_Cards_Three_High' or 'CasinoUI_Cards_Three'))
    ...
    Citizen.CreateThread(function()
        while instruction_showed do
            ...
            RageUI.IsVisible(RMenu:Get('casino', 'main'), true, false, true, function()
                ...
                RageUI.Button('Jouer au Poker trois cartes', '', {RightLabel = '→→→'}, true, function(Hovered, Active, Selected)
                    if (Active) then
                        frm_showed = false
                    end
                ...
                RageUI.Button('Description du jeu', 'Comment fonctionne le jeu ?', {RightLabel = '→→→'}, true, function(Hovered, Active, Selected)
                    if (Active) then
                        frm_showed = false
                    end
                ...
                RageUI.Button('Règles du jeu', 'Règles du jeu ou tout ce que vous devez savoir.', {RightLabel = '→→→'}, true, function(Hovered, Active, Selected)
                    if (Active) then
                        frm_showed = false
                    end
...
function drawfreameeMarker(position)
    DrawMarker(20, position, 0.0, 0.0, 0.0, 180.0, 0.0, 0.0, 0.3, 0.3, 0.3, 0, 255, 0, 255, true, true, 2, true, nil, nil, false)
end

RMenu.Add('aquiver_poker', 'instructions', RageUI.CreateMenu('', 'test', 0, 100, 'CasinoUI_Cards_Three_High', 'CasinoUI_Cards_Three_High'))
RMenu:Get('aquiver_poker', 'instructions'):SetSubtitle('Three Card Poker')

function showHowTo()
    Citizen.CreateThread(
        function()
            local helps = {'TCP_DIS1', 'TCP_DIS2', 'TCP_DIS3'}
            ...
            for i = 1, #helps, 1 do
                ...
                EndTextCommandDisplayHelp(0, false, false, Config3Poker.HowToSpeed)

                Citizen.Wait(Config3Poker.HowToSpeed)
...
RegisterCommand(
    ...
    function()
        ...
        for i = 1, #Config3Poker.Tables, 1 do
            local obj = GetClosestObjectOfType(playercoords, 3.0, GetHashKey(Config3Poker.Tables[i]), false, false, false)
